...
public class ProductServiceImpl implements ProductService {
    ...
    @Transactional(propagation = Propagation.SUPPORTS)//不需要事务，可重复读
...
//            System.out.println(productImgs);
            ...
            return new ResultVO(ResStatus.OK,"商品基本信息查询成功",map);
        }else{
            return new ResultVO(ResStatus.NO,"该商品不存在",null);
        }
    }
    ...
    public ResultVO getProductParamsById(String productId) {
        ...
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("productId",productId);
        List<ProductParams> Params = productParamsMapper.selectByExample(example);
        if (Params.size()>0){
            return new ResultVO(ResStatus.OK,"商品参数查询成功",Params.get(0));
        }else {
            return new ResultVO(ResStatus.NO,"该商品无商品参数，三无产品",null);
        }
    }
